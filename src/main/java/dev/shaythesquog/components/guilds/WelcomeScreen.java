...
public class WelcomeScreen implements JsonAPIComponent {
     ...
     */
    public static class WelcomeScreenChannel implements JsonAPIComponent {
        private final Snowflake channel_id;
        ...
        public WelcomeScreenChannel(JsonObject data) {
            channel_id = new Snowflake(data.get("channel_id").getAsString());
            description = data.get("description").getAsString();
            emoji_id = data.get("emoji_id").isJsonNull() ? null : new Snowflake(data.get("emoji_id").getAsString());
            emoji_name = data.get("emoji_name").isJsonNull() ? null : data.get("emoji_name").getAsString();
        }

        public JsonObject toJson() {
            JsonObject data = new JsonObject();
            data.addProperty("channel_id", channel_id.toString());
            data.addProperty("description", description);
